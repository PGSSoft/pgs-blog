...
class SearchCriteriaGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the SearchCriteria entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all searchCriteria")
            ...
            .headers(headers_http_authenticated)
            .body(StringBody("""{"id":null, "keyWord":"SAMPLE_TEXT", "mustHaveWord":"SAMPLE_TEXT", "excludedWord":"SAMPLE_TEXT"}""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_searchCriteria_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
